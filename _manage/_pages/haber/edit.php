...
<form name="birseyekle" method="POST" enctype="multipart/form-data" action="/admin/_manage/haber/edit.php" onSubmit="return checkfields(this)">
  ...
  <?
  			...
  			$get_haber = mysql_query("select id, kategori, baslik, spot, metin, konum, manset, resim_1, resim_2, resim_3, video, zaman, editor, okunmasayisi, ekhaber1, ekhaber2, ekhaber3, ozel from klist_haberler where id='$remhid'");
  			list($ed_id, $ed_kat, $ed_bas, $ed_spot, $ed_met, $ed_konum, $ed_man, $ed_res1, $ed_res2, $ed_res3, $ed_video, $ed_zaman, $ed_editor, $ed_os, $ed_eh1, $ed_eh2, $ed_eh3, $ozel)=mysql_fetch_row($get_haber);
     ...
     <tr>
      ...
      <td align="left" valign="top">Konum</td>
       ...
       <td width="90%" colspan="2">
				...
				<font face="Verdana" size="1">
						<select size="1" name="konumum" style="font-family: Arial; font-size: 10px; border-style: solid; border-width: 1; padding-left: 1; padding-right: 1; padding-top: 1; padding-bottom: 1">
 						
 						<? $katix = mysql_query("select id,konum from klist_konumlar where id='$ed_konum'");
 						list($konid, $kon) = mysql_fetch_row($katix);
 						?>
 						<option selected value="<?=$konid?>"><?=$kon?></option>
          <?

			$lista = mysql_query("select id,konum from klist_konumlar order by id");
			while(list($idy,$caty)=mysql_fetch_row($lista)){
...
?>
    ...
    <tr>
      ...
      <td width="90%" colspan="2" bordercolor="#FF0000" bgcolor="#FF0000">
		<input type="text" name="baslik_a" size="70" value="<?=$ed_bas?>"></td>
    ...
    <tr>
      ...
      <td width="90%" colspan="2"><input type="text" name="spot" size="70" value="<?=$ed_spot?>"></td>
    ...
    <tr>
     ...
     <p>
			...
			<textarea class="ckeditor" cols="80" id="editor1" name="metin" rows="10"><?=$ed_met?></textarea>
    ...
    <tr>
      <td>Man&#351;et Foto</td>
      <td colspan="2"><input type="file" name="filename2" size="20" style="font-family: Tahoma; font-size: 11px" /></td>
    </tr>
    <tr>

      <td>Video</td>

      <td width="90%" colspan="2"><input type="text" name="video" size="100" value='<?=$ed_video?>'></td>
