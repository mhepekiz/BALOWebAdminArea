...
if($ac_t=="upd"){
	...
	$medya = mysql_real_escape_string($medya);
		...
		//	$baslik = addslashes($baslik);
			...
			mysql_query("update medyalogo set medya='$medya', zaman='$zaman', editor='$e_uname' where id='$remhid'")or die("DB Error");
...
$ressim = resim($filename_name);
  ...
  if($filename){
	...
	mysql_query("update medyalogo set resim_1='$detail_sql_name' where id='$remhid'") or die("DB Err");
		        

     
$proc = "Kayit guncellendi ($medya)...";



echo "<script> alert(\"Kayit guncellendi ($medya)!\"); </script>";
...
$welcomefile=$docroot."/admin/_manage/editor/medyalogo_edit.src.php";
include($welcomefile);
