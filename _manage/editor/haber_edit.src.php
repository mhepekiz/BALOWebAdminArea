<?
	...
	$haber_detay = mysql_query("select id, manset, lang, kategori, tarih, yayin, baslik, spot, metin from haberler where id='$remhid'");
	list($h_id, $h_mans, $h_lang, $h_cat, $h_tar, $h_yayin, $h_baslik, $h_spot, $h_metin) = mysql_fetch_row($haber_detay);
...
<form name="birseyekle" method="POST" enctype="multipart/form-data" action="/admin/_manage/editor/haber_edit.php" >
  ...
  <tr>
      ...
      <td valign="top" width="10%"><font face="Arial, Helvetica, sans-serif" style="font-size:12px">Küçük Foto&#287;raf</font></td>
...
<script>DateInput('orderdate', true, 'DD-MON-YYYY','<?=$trh[2]?>-<?=$trhay?>-<?=$trh[0]?>')</script>
  ...
  <tr>
      ...
      <td align="left">Man&#351;et</td>

      <td width="100%"><select name="manset" />
      <?
      
	  if($h_mans=="yes"){ echo'<option selected value="yes">Evet</option>'; } elseif($h_mans=="no"){ echo'<option selected value="no">Hay&#305;r</option>'; } 
